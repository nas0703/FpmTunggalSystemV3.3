...
export const HasilBulananTable = ({
  ...
  const calculateRowData = (blokId: string, isLF: boolean = false) => {
    ...
    const tPenMonth = (hhiMt / currentDay) * daysInMonth; // Estimation for month end
  ...
  const renderRow = (
    ...
    const yoyVal =
      data.ytd2025 > 0
        ...
        : 0;
    const yoyColor = yoyVal >= 0 ? "text-emerald-600" : "text-rose-600";
    ...
    return (
      <tr
        key={data.blok}
        className={`${rowClass} border-b border-emerald-200 dark:border-emerald-800 transition-colors ${!isRankedColor && !isGrandTotal && !isSubtotal ? "hover:bg-emerald-50 dark:hover:bg-emerald-900/20" : ""}`}
      >
        <td
          style={zoomStyles}
          className="border-r border-emerald-200 dark:border-emerald-800 text-center uppercase"
        >
          {data.pkt}
        </td>
        <td
        ...
        <td
          style={zoomStyles}
          className={`border-r border-emerald-200 dark:border-emerald-800 text-center ${isRankedColor ? "opacity-80" : "text-emerald-400/70"}`}
        >
          {data.targetTahun}
        </td>
        <td
          style={zoomStyles}
          className={`sticky left-0 border-r-2 border-emerald-400 dark:border-emerald-700 text-center font-black shadow-[2px_0_5px_rgba(0,0,0,0.1)] z-10 ${rowClass}`}
        >
          {data.blok}
        </td>
        <td
          style={zoomStyles}
          className="border-r border-emerald-200 dark:border-emerald-800 text-right pr-2 font-medium"
        >
          {data.luas.toFixed(2)}
        </td>
        <td
          style={zoomStyles}
          className={`border-r border-emerald-200 dark:border-emerald-800 text-center font-bold ${isRankedColor ? "opacity-80" : isDarkMode ? "text-rose-400" : "text-rose-600"}`}
        >
        ...
        <td
          style={zoomStyles}
          className={`border-r border-emerald-200 dark:border-emerald-800 text-right pr-2 ${!isRankedColor && !isGrandTotal && !isSubtotal ? "bg-emerald-50/30 dark:bg-emerald-900/20" : ""}`}
        >
          {data.anggaranMonth.mt.toFixed(2)}
        </td>
        <td
          style={zoomStyles}
          className={`border-r border-emerald-200 dark:border-emerald-800 text-center ${!isRankedColor && !isGrandTotal && !isSubtotal ? "bg-emerald-50/30 dark:bg-emerald-900/20" : ""}`}
        >
        ...
        <td
          style={zoomStyles}
          className="border-r border-emerald-200 dark:border-emerald-800 text-right pr-2 font-black"
        >
        ...
        <td
          style={zoomStyles}
          className="border-r border-emerald-200 dark:border-emerald-800 text-center font-black"
        >
          {data.pencapaianMonth.tHek.toFixed(2)}
        </td>
        <td
          style={zoomStyles}
          className={`border-r border-emerald-200 dark:border-emerald-800 text-center font-black ${isRankedColor ? "" : "text-emerald-600 dark:text-emerald-400"}`}
        >
  ...
  const calculateGroupTotal = (rows: any[], label: string) => {
    ...
    const totalTPenMonth = rows.reduce(
      (acc, curr) => acc + curr.pencapaianMonth.tPen,
      0,
    );
  ...
  const handleExportTableToExcel = async () => {
    try {
      ...
      ws.mergeCells("F5:F6");
      headerRow1.getCell(6).value = "CAPAI T/HEK (2025)";

      ws.mergeCells("G5:H5");
      headerRow1.getCell(7).value = "ANGGARAN BULAN INI";
      ws.mergeCells("I5:M5");
      headerRow1.getCell(9).value = "PENCAPAIAN SEHINGGA HARI INI";
      ws.mergeCells("N5:O5");
      headerRow1.getCell(14).value = "ANGGARAN HINGGA BULAN INI";
      ws.mergeCells("P5:S5");
      headerRow1.getCell(16).value = "PENCAPAIAN HINGGA BULAN INI";

      // 3. Headers Row 2
      const headerRow2 = ws.getRow(6);
      headerRow2.getCell(7).value = "MT";
      headerRow2.getCell(8).value = "T/HEK";

      headerRow2.getCell(9).value = "HI M/T";
      headerRow2.getCell(10).value = "HHI M/T";
      headerRow2.getCell(11).value = "T/HEK";
      headerRow2.getCell(12).value = "T.PEN M/T";
      headerRow2.getCell(13).value = "% CAPAI";

      headerRow2.getCell(14).value = "MT";
      headerRow2.getCell(15).value = "T/HEK";

      headerRow2.getCell(16).value = "M/T";
      headerRow2.getCell(17).value = "T/HEK";
      headerRow2.getCell(18).value = "% CAPAI";
      headerRow2.getCell(19).value = "YOY (%)";
      ...
      const appendRow = (
        ...
        row.eachCell((cell, colNumber) => {
          if (colNumber > 4) {
            cell.numFmt = "#,##0.00";
            if (colNumber === 13 || colNumber === 18 || colNumber === 19)
              cell.numFmt = '0.00"%"';
          }
    ...
    >
      <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end mb-6 gap-4 md:gap-6">
        <div className="flex items-center gap-3 md:gap-4 w-full lg:w-auto">
          ...
          <div className="flex flex-col">
            ...
            <h3 className="text-[9px] md:text-sm font-bold text-slate-500 mt-0.5 uppercase tracking-wide">
        ...
        <div className="flex flex-col xs:flex-row items-center justify-center gap-2 w-full lg:w-auto no-screenshot">
          <div className="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-800 p-0.5 md:p-1 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm max-w-full no-scrollbar">
            {[
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center justify-center w-7 h-7 md:w-8 md:h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white shadow-md shadow-emerald-500/20 transition-all active:scale-[0.95]"
                title="Muat Turun"
              >
              ...
              <AnimatePresence>
                {showDownloadMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-52 bg-white/95 dark:bg-slate-900/98 backdrop-blur-xl rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.15)] border border-slate-200 dark:border-slate-800 p-2 z-[100] ring-1 ring-black/5"
                  >
                    <div className="px-3 py-1.5 border-b border-slate-100 dark:border-slate-800/50 mb-1">
                    ...
                    <div className="space-y-0.5">
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-emerald-500/10 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all text-left group"
                      >
                        <div className="w-7 h-7 bg-emerald-500/10 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 transition-all text-left group"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-400 transition-all text-left group"
                      >
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-500/10 transition-all text-left group"
                    >
                      <div className="w-7 h-7 bg-slate-500/10 rounded-lg flex items-center justify-center text-slate-600 dark:text-slate-400 group-hover:bg-slate-600 group-hover:text-white transition-all">
                        <Printer size={14} />
                      </div>
                      <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1 bg-white dark:bg-slate-800 p-1 md:p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <button
              onClick={() => setZoom(Math.max(30, zoom - 10))}
              className="p-1.5 md:p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-lg transition-all text-slate-600 dark:text-slate-300 active:scale-95"
              title="Zum Keluar"
            >
              <ZoomOut size={16} />
            </button>
      ...
      <div
        ...
        className={`transition-all duration-300 w-full ${
          isFullscreen
            ? "fixed inset-4 md:inset-8 z-[9999] bg-white dark:bg-emerald-950 rounded-3xl shadow-2xl p-4 md:p-6 overflow-auto flex flex-col custom-scrollbar"
            : "bg-white dark:bg-emerald-950 border-2 border-emerald-900/30 dark:border-emerald-800 shadow-2xl shadow-emerald-900/10 overflow-x-auto custom-scrollbar sm:rounded-2xl cursor-pointer hover:border-emerald-500/50"
        }`}
      >
        {isFullscreen && (
          <div className="flex justify-between items-center mb-4 no-screenshot sticky left-0 top-0 z-50">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest leading-tight">
                Fullscreen Mode
                <br />
            ...
            <button
              ...
              className="p-2 flex items-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-full transition-all"
            >
              <X size={16} />
        ...
        <div
          ...
          className={
            isFullscreen
              ? "overflow-hidden flex-1 rounded-xl border border-slate-200 dark:border-emerald-800 scrollbar-hide flex items-center justify-center bg-slate-50/50 dark:bg-emerald-950/50"
              : ""
          }
        >
          <div
          ...
          >
            ...
            >
              <thead className="sticky top-0 bg-emerald-900 text-[10px] uppercase font-black tracking-wider text-white z-30 shadow-md">
                <tr>
                  <th
                    rowSpan={3}
                    style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}
                    className="border border-emerald-800 p-2 w-[4%] bg-emerald-900"
                  >
                    PKT
                  </th>
                  <th
                    rowSpan={3}
                    style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}
                    className="border border-emerald-800 p-2 w-[8%] bg-emerald-900"
                  >
                    THN TUAI
                  </th>
                  <th
                    rowSpan={3}
                    style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}
                    className="border border-emerald-800 p-2 w-[5%] bg-emerald-900"
                  >
                    TAR/MT
                  </th>
                  <th
                    ...
                    className="sticky left-0 border-x-2 border-emerald-700 p-2 w-[5%] bg-emerald-950 z-40 shadow-[2px_0_10px_rgba(0,0,0,0.3)]"
                  >
                    BLOK
                  </th>
                  <th
                    rowSpan={3}
                    style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}
                    className="border border-emerald-800 p-2 w-[6%] bg-emerald-900"
                  >
                    LUAS
                  </th>
                  <th
                    rowSpan={2}
                    style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}
                    className="border border-emerald-800 p-2 bg-emerald-900"
                  >
                    CAPAI
                  </th>
                  <th
                    ...
                    className="border border-emerald-800 p-2 bg-emerald-800"
                  >
                    BULAN INI
                  </th>
                  <th
                ...
                <tr style={{ fontSize: `${Math.max(8, 10 * ((zoom / 100) * renderScale))}px` }}>
                  <th
                    colSpan={2}
                    className="border border-emerald-700 p-1 bg-emerald-700"
                  >
                    ANGGARAN
                  </th>
                  <th
                  ...
                  <th
                    rowSpan={2}
                    className="border border-emerald-700 p-2 bg-emerald-700"
                  >
                    % CAPAI
                  </th>
                  <th
                ...
                <tr style={{ fontSize: `${Math.max(7, 9 * ((zoom / 100) * renderScale))}px` }}>
                  <th className="border border-emerald-800 p-1 min-w-[70px] bg-emerald-900">
                    APRIL 2025 <br />
                    T/HEK
                  </th>
                  <th className="border border-emerald-700 p-1 min-w-[60px] bg-emerald-800">
                    M/T
                  </th>
                  <th className="border border-emerald-700 p-1 min-w-[50px] bg-emerald-800">
                    T/HEK
                  </th>
                  <th className="border border-emerald-700 p-1 min-w-[60px] bg-emerald-800">
                    H.I M/T
              ...
              <tbody className="text-[10px] text-slate-800 dark:text-slate-200">
      ...
      <div className="mt-8 flex justify-between items-center text-[10px] text-emerald-600/60 dark:text-emerald-400/60 font-bold uppercase tracking-widest px-2">
        <div className="flex gap-6">
          <span className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-md bg-emerald-100 dark:bg-emerald-900/60 border border-emerald-300 dark:border-emerald-700 shadow-sm" />{" "}
            Sub-Total
          </span>
          <span className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-md bg-emerald-600 border border-emerald-500 shadow-md" />{" "}
            Jumlah Besar
          </span>
        </div>
